  ...
  const GROUPS = [
    ...
    },
    {
      ...
      cmds: [
        ...
        { name: 'Info dispositivo default',desc: 'Dispositivo ALSA configurado como default',               cmd: 'cat ~/.asoundrc 2>/dev/null; cat /etc/asound.conf 2>/dev/null; echo "---"; arecord --dump-hw-params -D default /dev/null 2>&1 | head -20' },
        ...
        { name: 'Formatos soportados mic',desc: 'Formatos y frecuencias soportadas por la tarjeta default',  cmd: 'arecord --dump-hw-params -D default /dev/null 2>&1' },
    ...
    {
      label: 'Speaker · Reproducción',
      icon: '🔈',
      desc: 'Altavoces ALSA, volumen y test de audio',
      cmds: [
        { name: 'Altavoces disponibles',    desc: 'Lista todos los dispositivos de reproducción ALSA',                                  cmd: 'aplay -l 2>&1' },
        { name: 'Info speaker plughw:0,0',  desc: 'Parámetros de hardware del altavoz en tarjeta 0 dispositivo 0',                      cmd: 'aplay --dump-hw-params -D plughw:0,0 /dev/null 2>&1 | head -25' },
        { name: 'Test tono 440 Hz',         desc: 'Reproduce un tono sinusoidal de 1 segundo por plughw:0,0 — confirma que el speaker funciona', cmd: 'speaker-test -D plughw:0,0 -t sine -f 440 -l 1 2>&1' },
        { name: 'Test tono — default',      desc: 'Mismo test por el dispositivo ALSA configurado como default',                        cmd: 'speaker-test -t sine -f 440 -l 1 2>&1' },
        { name: 'Test WAV grabado',         desc: 'Reproduce la última grabación de test (/tmp/test_brumexa.wav) por plughw:0,0',       cmd: 'aplay -D plughw:0,0 /tmp/test_brumexa.wav 2>&1 || echo "(no hay grabación de test — ejecutá primero «Test: grabar 3 segundos»)"' },
        { name: 'Grabar y reproducir',      desc: 'Graba 3s y los reproduce inmediatamente — verifica el ciclo mic→speaker completo',   cmd: 'arecord -d 3 -D plughw:0,0 -f S16_LE -r 16000 -c 1 /tmp/loopback.wav 2>&1 && echo "Reproduciendo..." && aplay -D plughw:0,0 /tmp/loopback.wav 2>&1' },
        { name: 'Detener aplay',            desc: 'Mata cualquier proceso aplay corriendo (útil si quedó colgado)',                     cmd: 'pkill -f "aplay" 2>/dev/null && echo "aplay detenido" || echo "no hay aplay corriendo"' },
        { name: 'Controles del speaker',    desc: 'Controles del mixer relacionados con reproducción (Master, PCM, Speaker)',           cmd: 'amixer sget Master 2>/dev/null; amixer sget PCM 2>/dev/null; amixer sget Speaker 2>/dev/null; amixer sget Headphone 2>/dev/null' },
        { name: 'Volumen al 80%',           desc: 'Sube el volumen de reproducción a 80% (Master o PCM)',                              cmd: 'amixer sset Master 80% 2>/dev/null && echo "Master → 80%" || amixer sset PCM 80% 2>/dev/null && echo "PCM → 80%" || echo "no se encontró control de volumen"' },
        { name: 'Volumen al 100%',          desc: 'Sube el volumen al máximo',                                                         cmd: 'amixer sset Master 100% 2>/dev/null && echo "Master → 100%" || amixer sset PCM 100% 2>/dev/null && echo "PCM → 100%" || echo "no se encontró control de volumen"' },
        { name: 'Procesos de audio activos',desc: 'Procesos aplay / arecord / pulseaudio corriendo ahora mismo',                       cmd: 'ps aux | grep -E "aplay|arecord|pulseaudio|pipewire" | grep -v grep || echo "(sin procesos de audio activos)"' },
        { name: 'ALSA → Google Voice HAT', desc: 'Formatos soportados por snd_rpi_googlevoicehat_soundcar (plughw:0,0)',               cmd: 'aplay --dump-hw-params -D hw:0,0 /dev/null 2>&1; echo "---"; arecord --dump-hw-params -D hw:0,0 /dev/null 2>&1' },
      ],
    },
    {
      label: 'Bluetooth',
