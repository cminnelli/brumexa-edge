 ...
 * rastro: todo el detector vive en este único archivo; en server.js y
 * public/configuracion.html cada línea de enganche (require, feed(), el
 * callback, el toggle de Configuración) está marcada con "CLAP-CONNECT" —
 * para desarmar la función, buscar ese tag y borrar + este archivo.
 * Activable/desactivable en caliente desde Configuración → Sonido →
 * Ajustes avanzados (sin código) — ver setEnabled()/getEnabled() abajo.
 *
...
// CLAP_CONNECT_ENABLED en .env — default true (no cambia nada para quien ya
// lo tenía andando) pero ajustable desde Configuración → Sonido → Ajustes
// avanzados (POST /setup/config → clapConnectEnabled) sin tocar código, por
// si en algún momento hace falta apagarlo rápido (falsos positivos con
// ruido de fondo, u otro motivo) sin depender de un despliegue nuevo.
let _armed         = process.env.CLAP_CONNECT_ENABLED !== 'false';
...
module.exports = { feed, onDoubleClap, setEnabled, getEnabled };
