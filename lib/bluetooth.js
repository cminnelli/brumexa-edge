  ...
  return new Promise((resolve) => {
    ...
    exec(cmd, { timeout: (seconds + 4) * 1000, shell: true }, (err, stdout) => {
      ...
      for (const line of devOut.split('\n')) {
        // Quitar ANSI escape codes por si acaso
        ...
        // Nombre = solo MAC en formato guiones → no es un dispositivo real
        if (/^([0-9A-F]{2}-){5}[0-9A-F]{2}$/i.test(name)) continue;
        devices.push({ mac, name, connected: isConnected(mac), paired: isPaired(mac) });
      }

      console.log(`[bluetooth] Resultado: ${devices.length} dispositivo(s) con nombre`);
      resolve(devices);
    });
  });
}
