...
let _filename  = null;
let _startedAt = null;
let _lastError = null; // { device, message, exitCode, at } — última falla de arecord, para diagnóstico
...
function applyGainToWav(filePath, gain, normTarget = 0.85) {
  ...
  try {
    ...
    // Ganancia total = gain fijo + normalización al normTarget del máximo
    const normGain   = (32767 * normTarget) / peak;
...
// normTarget: nivel objetivo de normalización [0.5..1.0], default 0.85 (≈ -1.4 dBFS)
function startRecording(device = 'default', normTarget = 0.85) {
  ...
  _proc.stderr.on('data', (d) => {
    ...
    if (msg) { console.log('[arecord]', msg); _stderrTail = msg; }
  });
  _proc.on('error', (err) => {
    console.error('[recorder] Error arecord:', err.message);
    _lastError = { device, message: err.message, exitCode: null, at: Date.now() };
    _proc = null; _filename = null; _startedAt = null;
  });
  _proc.on('close', (code) => {
    console.log(`[recorder] arecord terminó (código ${code}) → ${path.basename(dest)}`);
    // code === 0 (SIGINT limpio via stopRecording) o null (matado con señal) son
    // cierres normales. Cualquier otro código es una salida inesperada del
    // dispositivo (ocupado, desconectado, nombre inválido, etc.)
    if (code !== 0 && code !== null) {
      _lastError = {
        device,
        message: _stderrTail || `arecord salió con código ${code}`,
        exitCode: code,
        at: Date.now(),
      };
    }
    ...
    // normTarget capturado en closure — aplica ganancia + normalización al WAV
    const MIC_GAIN = parseFloat(process.env.MIC_GAIN) || 4.0;
    applyGainToWav(dest, MIC_GAIN, normTarget);
  });
