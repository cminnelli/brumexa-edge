...
// Mismo algoritmo que audio.js applyGain() pero opera sobre el archivo en disco.
// Salta el header WAV estándar (44 bytes) y procesa samples Int16 little-endian.
function applyGainToWav(filePath, gain) {
  if (!gain || gain <= 1.0) return;
  try {
    const buf = fs.readFileSync(filePath);
    if (buf.length < 44) return; // demasiado pequeño para ser WAV válido
    for (let i = 44; i + 1 < buf.length; i += 2) {
      const s = buf.readInt16LE(i);
      buf.writeInt16LE(Math.max(-32768, Math.min(32767, Math.round(s * gain))), i);
    }
    fs.writeFileSync(filePath, buf);
    console.log(`[recorder] gain x${gain} (${(20 * Math.log10(gain)).toFixed(1)} dB) aplicado → ${path.basename(filePath)}`);
  } catch (err) {
