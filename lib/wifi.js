...
// let, no const: setApSsid() más abajo la reasigna en caliente cuando se
// cambia desde /configuracion, sin esperar a un reinicio del proceso.
let AP_SSID      = process.env.WIFI_AP_SSID || os.hostname();
...
// Si el AP ya está activo con el nombre viejo, hay que reactivarlo con el
// nuevo para que el cambio se vea YA — eso corta por un instante a quien esté
// conectado a esa red (típicamente nadie, salvo el raro caso de estar
// cambiando el nombre desde /configuracion mientras se está conectado al
// propio AP en vez de al WiFi real).
async function setApSsid(newSsid) {
  const trimmed = (newSsid || '').trim();
  if (!trimmed) return { ok: false, error: 'El nombre del AP no puede estar vacío' };
  if (trimmed.length > 32) return { ok: false, error: 'Máximo 32 caracteres (límite de WiFi)' };
  if (trimmed === AP_SSID) return { ok: true, ssid: AP_SSID, restarted: false };

  const wasActive = getStatus().apActive; // chequear ANTES de pisar AP_SSID
  const oldSsid   = AP_SSID;
  AP_SSID = trimmed;
  _logDebug(`SSID del AP cambiado: "${oldSsid}" → "${AP_SSID}"`);

  if (!wasActive) return { ok: true, ssid: AP_SSID, restarted: false };

  run(`nmcli connection delete "${oldSsid}" 2>/dev/null || true`, 5000);
  const result = await startAP();
  return { ok: result.ok, ssid: AP_SSID, restarted: true, error: result.error };
}
