...
// Bajado de 45s a 20s — pedido explícito: reaccionar "en unos segundos", no
// casi un minuto. Sigue habiendo margen (2 ticks de 15s antes de esto,
// DISCONNECT_AP_RECOVERY_MS encima) para no saltar por un bache de un
// instante, solo que ahora es bastante más corto.
const DISCONNECT_AP_RECOVERY_MS = parseInt(process.env.WIFI_DISCONNECT_AP_RECOVERY_MS, 10) || 20000;
...
// ~ms) — pensado para el tick normal del monitor de salud, cada 15s, sin
// pagar el costo de un rescan real en cada uno. Con forceRescan=true (ver
...
async function _checkHealthOnce() {
  ...
  if (!status.connectedSSID || !status.ipAddress) {
    ...
    // ciclo de 15s en vez de depender de cuándo NetworkManager decida
