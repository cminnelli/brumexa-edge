...
async function autoStartAP() {
  ...
  if (!status.ipAddress && !status.apActive) {
    ...
    // Pedido explícito: sonar también acá, no solo en una reconexión en
    // caliente — "arranqué y ya tengo internet" también cuenta como "hay
    // WiFi" para quien lo está escuchando, no solo viendo la pantalla.
    try { require('./sound-effects').playWifiConnectedSound(); } catch (e) { console.warn('[wifi] playWifiConnectedSound:', e.message); }
  }
}
