...
function getServerLogTail(lines = 50) {
  ...
  const outFile = path.join(home, '.pm2', 'logs', `${PM2_APP_NAME}-out.log`);
  const errFile = path.join(home, '.pm2', 'logs', `${PM2_APP_NAME}-error.log`);
  return {
...
function setupLocalDebug(app, { getWifiStatus, lkSession, getMicLevel, getRecorderStatus, getDeviceConfig } = {}) {
