...
async function login() {
  if (!DEVICE_ID || !API_KEY) {
    throw new Error('BRUMEXA_DEVICE_ID / BRUMEXA_API_KEY no configurados en .env');
  }

  console.log(`[rag-auth] Autenticando dispositivo "${DEVICE_ID}" → ${RAG_API_URL}/auth/device`);
  const res = await fetch(`${RAG_API_URL}/auth/device`, {
    method:  'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(`[rag-auth] Login fallido (${res.status}): ${body?.error || 'error desconocido'}`);
  }
  ...
  scheduleRefresh(exp);
  console.log(`[rag-auth] ✔ Autenticado — negocio=${state.businessId}  expira=${new Date(exp).toLocaleString()}`);
}
