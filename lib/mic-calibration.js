...
// El pop del códec deja una cola que sigue decayendo bastante después de
// abrir el device — en una corrida real se vio la cola todavía audible
// (-8.6 → -11.2 → -15.7 → -24.8dBFS) hasta ~900ms desde que arrancó
// arecord. 500ms no alcanzaba a cubrirla entera; 1200ms le da margen.
const WARMUP_MS = 1200;

// Qué percentil de los ticks se usa como "piso de ruido" — con pocas
// muestras (la ventana es corta) 0.9 solo descarta 2-3 valores, que no
// siempre alcanza para tapar toda la cola del pop si el warm-up se queda
// corto — 0.75 descarta un cuarto de las lecturas, más red de seguridad
// sin dejar de ser representativo del ambiente real.
