...
// "Sensing" — aviso visual CASI instantáneo (2 muestras seguidas por
// encima del umbral, ~200ms — no las 3/~300ms del streak de
// ONSET_MIN_STREAK) para que el LED reaccione rápido, sin tener que bajar
// el umbral de confirmación real (eso reabriría la puerta a que un ruido
// corto dispare el gate de audio, que es justo lo que ONSET_MIN_STREAK
// evita — ver su comentario arriba). Nunca gatea audio ni cuenta como
// "está hablando" confirmado — es SOLO una señal de "algo se está
// escuchando" para que el LED no se sienta con delay, mientras la
// confirmación real (isVoiceActive(), lo único que abre el gate y dispara
// la animación completa) sigue exigiendo el streak entero. Antes exigía
// UNA sola muestra — en un ambiente con picos cortos (ver _updateMicAmbientFloor)
// eso hacía que el eco visual titilara con cada pico aislado en vez de
// reaccionar solo a algo que empieza a sostenerse; 2 muestras ya filtra el
// caso de "un solo sample subió por ruido eléctrico/RF" sin perder la
// sensación de instantáneo. Hangover propio y corto: si no llega a
// confirmarse, se apaga solo enseguida.
const SENSING_MIN_STREAK  = 2;
// Se probó subir esto a 600 (de 250) para suavizar el parpadeo cuando el
// nivel anda bordeando el umbral — funcionó para eso, PERO como efecto
// secundario la ola de entrada (_onsetAnim en leds.js) arranca su brillo
// desde _lastRGB, que con un hangover largo queda pisado en el brillo
// plano y bajo de sensing (SENSING_BRIGHTNESS) durante mucho más tiempo
// antes de que la confirmación real llegue — la ola se sentía "menos
// automática/inmediata" al arrancar a hablar, algo que SÍ se notaba y no
// gustó. Vuelto a 250 — el parpadeo en el borde queda como estaba, pero
// no se sacrifica la sensación de inmediatez al empezar a hablar, que es
// más importante. Si hay que volver a atacar el parpadeo, mejor un
// cooldown de re-armado específico en vez de estirar cuánto dura
// sostenido cada vez (eso es lo que rompió la ola).
...
function feed(level) {
  ...
  if (_aboveStreak >= SENSING_MIN_STREAK) {
    _sensing = true;
