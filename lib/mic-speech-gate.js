...
// Subido de 250 a 600 — con 250ms, cuando el nivel anda justo bordeando el
// umbral (no confirma voz real, solo roza), sensing se prendía y apagaba en
// ráfaga (arma en ~200ms, apaga en 250ms — casi el mismo ritmo), cada
// toggle dispara un fundido en el LED → se veía como un parpadeo tosco en
// vez de un eco suave. El armado sigue siendo rápido (SENSING_MIN_STREAK
// no cambia — "se siente instantáneo" no se toca), pero ahora que se prende
// se sostiene un rato más, así un par de muestras que dipean por debajo del
// umbral en el medio de un ruido sostenido no alcanzan para apagarlo y
// volver a prenderlo enseguida.
const SENSING_HANGOVER_MS = 600;
let _sensing       = false;
