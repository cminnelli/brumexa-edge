...
//
// Async (exec, no execSync): esto se disparaba en cada carga de
// Configuración (GET /audio-devices) — un execSync bloqueante ahí, aunque
// sea "una sola vez" por carga de página, se veía en logs reales como
// ~40ms de atraso del event loop justo en ese momento. Mismo patrón que
// ya se arregló en wifi.js/local-debug.js/configuracion.js.
async function listAlsaDevices() {
  let out;
  try {
    out = (await execAsync('arecord -l 2>&1', { timeout: 3000 })).stdout;
...
async function listAlsaPlaybackDevices() {
  let out;
  try {
    out = (await execAsync('aplay -l 2>&1', { timeout: 3000 })).stdout;
...
// Busca, dentro de una lista de listAlsaDevices()/listAlsaPlaybackDevices(),
// la tarjeta del HAT de audio (mic INMP441 + speaker MAX98357A) por su
// nombre de driver — "googlevoicehat", siempre el mismo sin importar en qué
// número de tarjeta haya quedado esta placa puntual (varía según qué otro
// audio tenga la Raspberry: una Zero 2W sin audio propio suele darle 0, pero
// una 3 A+ con jack/HDMI onboard puede correrlo a 1 o más). Devuelve el id
// listo para usar (ej. "plughw:1,0") o null si no aparece.
function findHatDevice(devices) {
  const hat = devices.find(d => /googlevoicehat/i.test(d.name));
  return hat ? hat.id : null;
}

// ─── Registrar endpoints en Express + WebSocket ───────────────────────────────
// extraWsPaths: { '/otro/path': (ws, req) => {...} } — este módulo es el
// dueño único del 'upgrade' del httpServer (ver el comentario más abajo
// sobre el bug de ws v8 con dos WebSocketServer con distinto path en el
// mismo server), así que cualquier OTRO WebSocket de la app (ej. /ws/logs,
// ver lib/log-stream.js) tiene que sumarse acá en vez de abrir su propio
// WebSocketServer independiente.
function setupAudio(app, httpServer, extraWsPaths = {}) {

  // GET /audio-devices — lista de dispositivos ALSA de captura
  app.get('/audio-devices', async (_req, res) => {
    const devices = await listAlsaDevices();
  ...
  app.get('/audio-playback-devices', async (_req, res) => {
    const devices = await listAlsaPlaybackDevices();
  ...
  httpServer.on('upgrade', (req, socket, head) => {
    ...
    if (pathname === '/ws/audio' || pathname === '/ws/speaker' || extraWsPaths[pathname]) {
  ...
  wss.on('connection', (ws, req) => {
    ...
    } else if (extraWsPaths[pathname]) {
      extraWsPaths[pathname](ws, req);
    }
  });

  const extraNames = Object.keys(extraWsPaths).join(', ');
  console.log(`[audio] Listo — /audio-devices, /audio-playback-devices, /ws/audio, /ws/speaker${extraNames ? ', ' + extraNames : ''}`);
}

module.exports = { setupAudio, getMicGain, setMicGain, listAlsaDevices, listAlsaPlaybackDevices, findHatDevice };
