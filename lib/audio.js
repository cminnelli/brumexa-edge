...
// Lee /proc/asound/pcm directamente — sin parsear texto humano, sin regex.
// Cada línea tiene el formato: "00-00: Nombre del dispositivo : ... : capture N"
function listAlsaDevices() {
  try {
    const lines = fs.readFileSync('/proc/asound/pcm', 'utf8').trim().split('\n');

    return lines
      .filter((line) => line.includes('capture'))   // solo dispositivos de entrada
      .map((line) => {
        const [cardDev, name] = line.split(':');     // "00-00" y " Nombre ..."
        const [card, device]  = cardDev.trim().split('-').map(Number);
        return {
          id:     `hw:${card},${device}`,
          card,
          device,
