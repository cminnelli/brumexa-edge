...
// sólido. IDLE_HUE/SAT quedan pisados hasta que setDeviceColor() los
// restaure (el módulo del panel lo hace solo al cerrar, vía
// /diag/leds/set/exit → setDeviceColor(getDeviceColor())).
...
function _createSpeechTracker(label, getHue, getSat, levelGain, hangoverMs) {
  ...
  function loop() {
    ...
    _breathe = setInterval(() => {
      ...
      const { r, g, b } = hsvToRgb(getHue(), getSat(), smoothed);
      _render(r, g, b);
    }, 16);
  }
  ...
  return function feed(level) {
    ...
    if (!active) {
      ...
      if (ws281x) {
        const from = _lastRGB;
        const to   = hsvToRgb(getHue(), getSat(), _speechBrightness(level, levelGain));
