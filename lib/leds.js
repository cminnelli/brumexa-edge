...
// — 'sweep' lo recorre un extremo tras otro (0..NUM_LEDS-1); 'centerOut'
// solo llega hasta la distancia del LED más alejado del centro (la mitad
// del rango, ya que los dos lados avanzan a la vez). Hace falta saber esto
// de antemano para totalMs — sin la forma correcta, la ola termina
// "antes de tiempo" (centerOut tiene menos order máximo que sweep) o
// "tarde" (le sobra tiempo esperando un order que ningún LED tiene).
function _waveMaxOrder(shape) {
  return shape === 'centerOut' ? (NUM_LEDS - 1) / 2 : NUM_LEDS - 1;
}
...
function _playSpeechWave(fromHue, fromSat, fromV, toHue, toSat, toV, { staggerMs, riseMs, reverse, shape = 'sweep' }, onDone) {
  ...
  _breathe = setInterval(() => {
    ...
    for (let i = 0; i < NUM_LEDS; i++) {
      const order  = shape === 'centerOut'
        ? Math.abs(i - (NUM_LEDS - 1) / 2)          // 3.5,2.5,1.5,0.5,0.5,1.5,2.5,3.5 — LEDs del medio primero
        : (reverse ? (NUM_LEDS - 1 - i) : i);         // barrido lineal, como siempre
...
// Salida ("dejaste de hablar"): probando "se abre" (centerOut) a pedido
// explícito — arranca en el medio de la tira y se abre hacia los dos
// extremos a la vez, en vez del barrido lineal 7→0 que tenía antes. Para
// volver al barrido de siempre: shape: 'sweep' (reverse:true ya queda
// listo para eso, no se tocó).
...
function _playSpeechOffset(fromHue, fromSat, fromV, toHue, toSat, toV, onDone) {
  _playSpeechWave(fromHue, fromSat, fromV, toHue, toSat, toV,
    { staggerMs: SPEECH_OFFSET_STAGGER_MS, riseMs: SPEECH_OFFSET_RISE_MS, reverse: true, shape: 'centerOut' }, onDone);
}
