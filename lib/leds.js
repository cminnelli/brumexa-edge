...
// Configurable desde /configuracion (setHangoverMs) — leída EN VIVO por
// getHangoverMs() en vez de capturada una vez al crear el tracker (ver
// _createSpeechTracker más abajo, que la crea UNA sola vez al arrancar el
// módulo), así un cambio se aplica ya mismo, no recién en el próximo
// reinicio.
...
// no lo necesita. getHangoverMs: función (no un número) por la misma razón
// que getHue/getSat/getThreshold — se lee EN VIVO en cada setTimeout, así
// setHangoverMs() desde /configuracion se aplica ya, sin esperar a que este
// tracker (creado una sola vez al arrancar el módulo) se vuelva a crear.
function _createSpeechTracker(label, getHue, getSat, levelGain, getHangoverMs, getThreshold, onLevel) {
...
const speaking      = _createSpeechTracker('vos',    () => SPEAK_HUE,       () => SPEAK_SAT,       MIC_LEVEL_GAIN,   getHangoverMs, _getMicThreshold,       _updateMicAmbientFloor);
const agentSpeaking  = _createSpeechTracker('agente', () => AGENT_SPEAK_HUE, () => AGENT_SPEAK_SAT, AGENT_LEVEL_GAIN, getHangoverMs, () => SPEAK_THRESHOLD, null);
