...
// Llamar con level 0-1 (peak del mic) — LEDs pulsan con la voz
function speaking(level) {
  if (!ws281x) return;
  _stopBreathe();

  const brightness = Math.min(1, level * 2);  // amplifica un poco el rango
  const val        = Math.round(255 * brightness);
  const pixels     = new Uint32Array(NUM_LEDS);
  pixels.fill((0 << 16) | (val << 8) | val);  // cyan: G+B
  ws281x.render(pixels);
}
