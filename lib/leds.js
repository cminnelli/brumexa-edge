...
// transición de color real. Interpolando hue/sat/brillo y recién ahí
// convirtiendo a RGB (con la fórmula de ESTE archivo, la misma que usan
// breathe()/los trackers de habla, para que no haya salto contra ellos) el
// camino pasa por los hues intermedios de la rueda, nunca por gris.
//
...
function _fadeHue(fromHue, fromSat, fromV, toHue, toSat, toV, durationMs, onDone) {
  _stopBreathe();
  const deltaHue = ((toHue - fromHue + 540) % 360) - 180; // camino corto, con signo
  let t = 0;
  const STEP = 16;
  _breathe = setInterval(() => {
    t += STEP;
    const ease = _easeInOut(Math.min(1, t / durationMs));
    const hue  = (fromHue + deltaHue * ease + 360) % 360;
...
const SPEECH_ONSET_STAGGER_MS = 22;  // delay entre el arranque de un LED y el siguiente
const SPEECH_ONSET_RISE_MS    = 220; // cuánto tarda CADA LED en llegar a destino una vez que le toca

function _playSpeechOnset(fromHue, fromSat, fromV, toHue, toSat, toV, onDone) {
  if (!ws281x) { if (onDone) onDone(); return; }
  _stopBreathe();
  const deltaHue  = ((toHue - fromHue + 540) % 360) - 180;
  ...
  _breathe = setInterval(() => {
    ...
    for (let i = 0; i < NUM_LEDS; i++) {
      ...
      const hue    = (fromHue + deltaHue * eased + 360) % 360;
      const sat    = fromSat + (toSat - fromSat) * eased;
      const v      = fromV + (toV - fromV) * eased;
      colors[i] = hsvToRgb(hue, sat, v);
    }
