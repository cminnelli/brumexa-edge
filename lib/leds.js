...
// realmente se note el cambio al elegir otro color de carcasa. Restart-only
// (igual que el resto de estos hues): se lee una sola vez al arrancar el
// proceso — no hace falta que cambiar el color del dispositivo sea "en
...
const DEVICE_COLOR = colorSchemes.colors[process.env.BRUMEXA_COLOR]
  ? process.env.BRUMEXA_COLOR
  : (colorSchemes.defaultColor || 'negro');
const _activeLeds  = colorSchemes.colors[DEVICE_COLOR].leds;
const IDLE_HUE         = _activeLeds.respiracion.hue;
const IDLE_SAT         = _activeLeds.respiracion.sat ?? 1;
const SPEAK_HUE        = _activeLeds.voz.hue;
const SPEAK_SAT        = _activeLeds.voz.sat ?? 1;
const AGENT_SPEAK_HUE  = _activeLeds.agente.hue;
const AGENT_SPEAK_SAT  = _activeLeds.agente.sat ?? 1;
...
function _createSpeechTracker(label, hue, sat, levelGain, hangoverMs) {
  ...
  function loop() {
    ...
    _breathe = setInterval(() => {
      ...
      const { r, g, b } = hsvToRgb(hue, sat, smoothed);
      _render(r, g, b);
    }, 16);
  }
  ...
  return function feed(level) {
    ...
    if (!active) {
      ...
      if (ws281x) {
        const from = _lastRGB;
        const to   = hsvToRgb(hue, sat, _speechBrightness(level, levelGain));
