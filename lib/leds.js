...
// ─── Calibración de ruido ambiente: aviso + espera en el color del rol
// "calibracion" (blanco por default) ─────────────────────────────────────
// 3 blinks ("1, 2, 3, ya arranco") y después una respiración suave y
...
// solo lee sharedGate.isOpen() para animar. getHangoverMs: función (no un
...
function _createSpeechTracker(label, getHue, getSat, levelGain, getHangoverMs, getThreshold, sharedGate) {
  ...
  function _onsetAnim(level, threshold) {
    ...
    if (ws281x) {
      ...
      if (_waveInFlight) {
        // Volviste a hablar mientras la ola de "dejaste de hablar" seguía a
        // mitad de camino (paciencia/hangover corto + pausas cortas al
        // hablar hacen esto frecuente) — la tira tiene 8 colores distintos
        // ahora mismo, no uno solo, así que animar OTRA ola desde ahí daría
        // un salto de color raro. Se salta directo al color sólido de
        // "hablando" en vez de arrancar una ola sobre un estado que no se
        // puede reconstruir bien.
        _stopBreathe();
        _waveInFlight = false;
  ...
  function _offsetAnim() {
    ...
    if (ws281x) {
      ...
      if (_waveInFlight) {
        // Espejo del caso de arriba: dejaste de hablar mientras la ola de
        // "empezaste a hablar" todavía estaba a mitad de camino.
        _stopBreathe();
        _waveInFlight = false;
  ...
  if (sharedGate) {
    ...
    // LiveKit, ver _publishMic) — este tracker solo REFLEJA isOpen() en la
    // animación, sin volver a alimentar el detector (lo alimenta quien
