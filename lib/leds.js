...
// Respiracion verde rapida — modo hablando
function _breatheSpeaking() {
  _stopBreathe();
  let t = 0;
  const PERIOD_MS = 1800;  // mas rapido que idle
  const STEP_MS   = 33;
...
// Si hay voz → breathe verde. Si silencio → vuelve a rainbow.
...
function speaking(level) {
  if (!ws281x) return;
  console.log(`[leds] mic level=${level.toFixed(2)}`);

  if (level > SPEAK_THRESHOLD) {
    if (!_isSpeaking) {
      console.log('[leds] hablando → breathe verde');
      _isSpeaking = true;
      _breatheSpeaking();
    }
    ...
    _speakingTimeout = setTimeout(() => {
      console.log('[leds] silencio → breathe rainbow');
      _isSpeaking = false;
      breathe();
    }, 2500);
  }
