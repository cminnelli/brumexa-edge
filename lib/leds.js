...
// Ámbar/dorado del cometa de "arrancando" (booting) — distinto del cian de
// connecting() y del blanco de calibrating(), y más rápido/corto que ambos:
// es lo primero que se ve apenas init() corre, antes de que WiFi/audio/
// calibración terminen de armarse, así que tiene que leerse como "algo ya
// está pasando" de un vistazo, no como una respiración más.
const BOOT_HUE        = 40;
const BOOT_PERIOD_MS  = 500;    // tiempo de punta a punta (LED 0 → LED 7) — más rápido que COMET_PERIOD_MS
const BOOT_TAIL_LEN   = 2.5;
const BOOT_EXPONENT   = 1.6;
// Salvavidas: si a los 15s nadie tomó la tira todavía (calibrating() no
// arrancó — ej. autoDetectAlsaDevices()/boostCaptureGain() explotaron antes
// de llegar a runCalibration()), mejor caer a la respiración normal que
// quedar con el cometa de "arrancando" pegado para siempre. El arranque
// real (WiFi + audio + calibración) tarda unos segundos, no esto.
const BOOT_FALLBACK_MS = 15000;
...
//
// Antes eran 3 parpadeos secos ("1, 2, 3, ya arranco") — se leía genérico,
// como cualquier loading spinner. Ahora son 2 barridos de cometa de punta a
// punta (con rebote — tira RECTA de 8 LEDs, nunca wrap) como gesto de
// "despertando", mismo espíritu que booting() pero en el color de
// calibración en vez del de boot. Mismo tiempo total que los 3 parpadeos de
// antes (1200ms) para no alargar el arranque real.
const CALIBRATION_WAKE_PERIOD_MS = 600;  // un barrido de punta a punta (LED 0 → LED 7)
const CALIBRATION_WAKE_SWEEPS    = 2;    // barridos completos antes de pasar al pulso sostenido
const CALIBRATION_WAKE_TAIL_LEN  = 4;
const CALIBRATION_WAKE_EXPONENT  = 1.4;
// Cuánto dura el aviso de "despertando" antes de que server.js arranque a
// grabar de verdad — exportado para que quien orquesta la calibración
// espere EXACTO este tiempo en vez de duplicar el número a mano.
const CALIBRATION_COUNTDOWN_MS = CALIBRATION_WAKE_PERIOD_MS * CALIBRATION_WAKE_SWEEPS;
...
function init() {
  try {
    ...
    _lastError = null;
    booting(); // cometa ámbar — "arrancando", no la respiración normal todavía
  } catch (e) {
...
// ─── Arrancando: cometa ámbar, más rápido y corto que connecting() ──────────
// Lo primero que se ve tras init() — todavía no hay WiFi ni calibración ni
// sesión, así que no puede parecerse a NINGUNO de esos estados (ni al idle,
// que recién arranca cuando termina la calibración de boot). Se llama desde
// init() en vez de breathe() directo, y server.js ahora llama a init() lo
// antes posible (antes de armar WiFi/audio/rutas), así que esto es lo
// primero que ve el usuario después de enchufar, no la respiración normal.
function booting() {
  _startComet({
    periodMs: BOOT_PERIOD_MS, tailLen: BOOT_TAIL_LEN, exponent: BOOT_EXPONENT,
    colorFn: b => hsvToRgb(BOOT_HUE, 1, b),
  });
  const myInterval = _breathe; // _startComet ya lo asignó de forma síncrona
  setTimeout(() => {
    if (_breathe === myInterval) breathe(); // nadie tomó la tira todavía — ver BOOT_FALLBACK_MS
  }, BOOT_FALLBACK_MS);
}
...
// ─── Calibración de ruido ambiente: "despertando" (barrido cometa) + espera
// en el color del rol "calibracion" (blanco por default) ────────────────
// CALIBRATION_WAKE_SWEEPS barridos de punta a punta ("ya voy a medir") y
// después una respiración suave y sostenida mientras server.js graba el
// tramo de silencio — la respiración sigue corriendo indefinidamente hasta
// que se llama a calibrationDone(), server.js no tiene que sincronizar nada
// más que esperar CALIBRATION_COUNTDOWN_MS antes de empezar a grabar.
function calibrating() {
  if (!ws281x) return;
  _startComet({
    periodMs: CALIBRATION_WAKE_PERIOD_MS, tailLen: CALIBRATION_WAKE_TAIL_LEN,
    exponent: CALIBRATION_WAKE_EXPONENT, colorFn: _calibColor,
  });
  setTimeout(() => {
    _startPulse({
  ...
  if (sharedGate) {
    ...
    return function feed(level) {
      ...
      if (nowSensing) {
        ...
        _fadeTo(_lastRGB.r, _lastRGB.g, _lastRGB.b, to.r, to.g, to.b, SENSING_FADE_MS);
      }
    };
  }
...
module.exports = {
  init, on, off, breathe, connecting, booting, brumexaError, wifiWeak,
