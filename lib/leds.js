...
//
// "voz" (tu voz) es ámbar (hue 40) en purpura/verde/negro — a propósito:
// antes tenía un hue cercano al de "respiracion" (250 vs 270, 130 vs 150)
// que con el hsvToRgb de este archivo cae en el MISMO segmento de la rueda
// (mismo patrón r/g/b, solo cambia la proporción) → se veían como el mismo
// color, apenas un matiz distinto, así que costaba distinguir a simple vista
// "está respirando" de "te está escuchando". El ámbar cae en un segmento
// distinto del hue de respiración en esas 3 variantes → contraste real (no
// solo numérico).
// Excepción a propósito en "blanco": voz es blanco mate (sat:0) en vez de
// ámbar (pedido explícito) — igual mantiene el contraste real contra la
// respiración celeste (#68c5dd) de esa variante, así que no reintroduce el
// problema que el ámbar fijo evita en las demás.
//
...
// esto simplemente llega a destino y se queda quieto). Tres formas
// posibles, elegidas por shape:
//   - 'sweep' (reverse=false): barre LED 0→7 — la usa _playSpeechOnset
//     (entrada: "empezaste a hablar").
//   - 'sweep' (reverse=true): barre 7→0, "retrocediendo" — la forma
//     ORIGINAL de _playSpeechOffset, todavía disponible (no se borró).
//   - 'centerOut': arranca en el MEDIO de la tira (LEDs 3 y 4,
//     equidistantes) y se abre simétricamente hacia los dos extremos a la
//     vez — un efecto de "apertura" en vez de barrido lineal. Probando
//     esto en _playSpeechOffset a pedido explícito, en paralelo a la forma
//     de barrido de siempre — queda a un cambio de una línea volver atrás
//     si no convence.
//
...
// maxOrder: el mayor "order" que puede tocarle a algún LED, según la forma
// — 'sweep' lo recorre un extremo tras otro (0..NUM_LEDS-1); 'centerOut' y
// 'centerIn' solo llegan hasta la distancia del LED más alejado del centro
// (la mitad del rango, ya que los dos lados avanzan a la vez). Hace falta
// saber esto de antemano para totalMs — sin la forma correcta, la ola
// termina "antes de tiempo" (menos order máximo que sweep) o "tarde" (le
// sobra tiempo esperando un order que ningún LED tiene).
function _waveMaxOrder(shape) {
  return (shape === 'centerOut' || shape === 'centerIn') ? (NUM_LEDS - 1) / 2 : NUM_LEDS - 1;
}

function _playSpeechWave(fromHue, fromSat, fromV, toHue, toSat, toV, { staggerMs, riseMs, reverse, shape = 'sweep' }, onDone) {
  if (!ws281x) { if (onDone) onDone(); return; }
  _stopBreathe();
  _waveInFlight = true;
  const fromStd  = _toStdHue(fromHue);
  const toStd    = _toStdHue(toHue);
  // Un extremo con sat=0 (blanco puro, ej. "voz" en la combinación Blanco)
  // NO TIENE hue de verdad — _toStdHue(null) igual devuelve un número
  // (trata null como 0), así que sin este chequeo el barrido de abajo
  // interpretaba eso como "hay que recorrer hasta tal color" y de verdad
  // pasaba por un tercer tono (ej. un verde pálido entre celeste y blanco)
  // que no es NI el origen NI el destino — se leía como "hay varios
  // colores" en la transición en vez de solo dos. Con un lado sin color, el
  // hue se queda FIJO en el del OTRO lado (el único que sí importa): la
  // ola se ve como ese color perdiendo saturación hacia blanco, nunca
  // rotando por la rueda.
  const hueMatters   = fromSat > 0 && toSat > 0;
  const renderFromStd = hueMatters ? fromStd : (toSat === 0 ? fromStd : toStd);
  const deltaHue = hueMatters ? (((toStd - fromStd + 540) % 360) - 180) : 0;
  const totalMs  = staggerMs * _waveMaxOrder(shape) + riseMs;
  // Cuando los dos hues están LEJOS en la rueda (ej. violeta idle → ámbar
  // hablando, ~130-150°), el camino más corto pasa geométricamente por el
  // medio de la rueda — fucsia/rosa, un color que no es "tono de" ninguno
  // de los dos extremos, se ve como que la ola se "equivocó" de color a
  // mitad de camino. En vez de evitar el barrido de hue (perdería la
  // sensación de ola continua) se lo desatura en el punto medio del salto
  // — pasa por un tono PÁLIDO ahí en vez de un fucsia bien saturado, así
  // nunca se lee como "un color equivocado", solo se atenúa un instante.
  // Saltos chicos (≤60°, dentro de la misma familia de color) no se tocan
  // — dipStrength da 0 y el resultado es idéntico a antes. Con hueMatters
  // false (un lado sin color) deltaHue ya es 0, así que dipStrength da 0
  // solo — no hace falta el dip, no hay barrido de hue del que protegerse.
  const hueSwingDeg = Math.abs(deltaHue);
  const dipStrength = Math.max(0, Math.min(1, (hueSwingDeg - 60) / 120));
  let t = 0;
  const STEP = 16;
  _breathe = setInterval(() => {
    t += STEP;
    ...
    for (let i = 0; i < NUM_LEDS; i++) {
      // centerOut: distancia AL centro — LEDs del medio primero (abre).
      // centerIn: espejo — maxOrder MENOS esa distancia, así el LED del
      // medio (distancia mínima) le toca el order MÁS ALTO (último) y las
      // puntas (distancia máxima) el order MÁS BAJO (primero) — cierra
      // desde las puntas hacia el medio, lo opuesto exacto de centerOut.
      const centerDist = Math.abs(i - (NUM_LEDS - 1) / 2); // 3.5,2.5,1.5,0.5,0.5,1.5,2.5,3.5
      const order  = shape === 'centerOut' ? centerDist
        : shape === 'centerIn' ? _waveMaxOrder(shape) - centerDist
        : (reverse ? (NUM_LEDS - 1 - i) : i); // 'sweep', barrido lineal de siempre
      ...
      const hue    = (renderFromStd + deltaHue * eased + 360) % 360;
      const satDip = 1 - dipStrength * 0.6 * Math.sin(eased * Math.PI); // 1 en los extremos, mínimo (0.4×) a mitad de camino
      const sat    = (fromSat + (toSat - fromSat) * eased) * satDip;
...
function _playSpeechOnset(fromHue, fromSat, fromV, toHue, toSat, toV, onDone) {
  _playSpeechWave(fromHue, fromSat, fromV, toHue, toSat, toV,
    { staggerMs: SPEECH_ONSET_STAGGER_MS, riseMs: SPEECH_ONSET_RISE_MS, reverse: false, shape: 'centerOut' }, onDone);
}
...
// Salida ("dejaste de hablar"): "cierra" (centerIn) — espejo exacto de la
// entrada, que ahora "abre" (centerOut, ver _playSpeechOnset): las puntas
// cambian primero y convergen hacia el medio al final, como si la
// apertura de la entrada se estuviera cerrando. Para volver al barrido
// lineal de siempre: shape: 'sweep' (reverse:true ya queda listo para
// eso, no se tocó).
...
function _playSpeechOffset(fromHue, fromSat, fromV, toHue, toSat, toV, onDone) {
  _playSpeechWave(fromHue, fromSat, fromV, toHue, toSat, toV,
    { staggerMs: SPEECH_OFFSET_STAGGER_MS, riseMs: SPEECH_OFFSET_RISE_MS, reverse: true, shape: 'centerIn' }, onDone);
}
...
// 'ok' (breathe cian/magenta) | 'weak' (naranja) | 'ap' (AP de aprovisionamiento
// activo a propósito — breathe normal, NO es un error) | 'disconnected' (rojo fijo)
...
function _idleLandingColor() {
  if (_networkHealth === 'ok' || _networkHealth === 'ap') return hsvToRgb(IDLE_HUE, IDLE_SAT, IDLE_FLOOR);
  return { r: 0, g: 0, b: 0 };
}
...
function _idleLandingHueSat() {
  if (_networkHealth === 'ok' || _networkHealth === 'ap') return { h: IDLE_HUE, s: IDLE_SAT, v: IDLE_FLOOR };
...
function setNetworkHealth(state) {
  if (state !== 'ok' && state !== 'weak' && state !== 'ap' && state !== 'disconnected') return;
