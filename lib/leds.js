...
function speaking(level) {
  if (level > SPEAK_THRESHOLD) {
    ...
    _speakingTimeout = setTimeout(() => {
      ...
      if (ws281x) _fadeTo(0, 255, 0, 0, 0, 0, 600, breathe);
    }, 300);
  }
}

// ─── Error: respiración roja rápida, vuelve a breathe si se pasa durationMs ──
function brumexaError(durationMs) {
  ...
  if (durationMs) setTimeout(() => { _fadeTo(255, 0, 0, 0, 0, 0, 600, breathe); }, durationMs);
}
