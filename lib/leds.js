...
// carcasa Brumexa — se elige en /configuracion, BRUMEXA_COLOR en .env). Hoy
// las 4 variantes apuntan a EXACTAMENTE los mismos hues — no cambia nada
// visualmente todavía — el mapa existe para que asignarle una paleta propia
// a cada color de carcasa el día que se pida sea editar una fila acá, no
// tocar lógica de render. Restart-only (igual que el resto de estos hues):
// se lee una sola vez al arrancar el proceso, como cualquier otro const de
// esta sección — no hace falta que cambiar el color del dispositivo sea "en
// caliente" como el mic/speaker gain, es un dato que se fija una vez en el
// setup del equipo.
const COLOR_SCHEMES = {
  purpura: { idleHue: 234, speakHue: 40, agentSpeakHue: 120 },
  verde:   { idleHue: 234, speakHue: 40, agentSpeakHue: 120 },
  negro:   { idleHue: 234, speakHue: 40, agentSpeakHue: 120 },
  blanco:  { idleHue: 234, speakHue: 40, agentSpeakHue: 120 },
};
const DEVICE_COLOR = COLOR_SCHEMES[process.env.BRUMEXA_COLOR] ? process.env.BRUMEXA_COLOR : 'purpura';
const { idleHue: IDLE_HUE, speakHue: SPEAK_HUE, agentSpeakHue: AGENT_SPEAK_HUE } = COLOR_SCHEMES[DEVICE_COLOR];
...
function _createSpeechTracker(label, hue, levelGain, hangoverMs) {
  ...
  return function feed(level) {
    ...
    if (!active) {
      ...
      // cambia lo que se imprime. Threshold Y level juntos en la misma línea
