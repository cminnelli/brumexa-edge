...
function _startComet({ periodMs, tailLen, exponent, hue }) {
  if (!ws281x) return;
  _stopBreathe();
  const STEP = 16;
  let pos = 0;
  ...
  _breathe = setInterval(() => {
    ...
    for (let i = 0; i < NUM_LEDS; i++) {
      ...
      colors[i] = hsvToRgb(hue, 1, bright);
    }
...
function connecting() {
  _startComet({
    periodMs: COMET_PERIOD_MS, tailLen: COMET_TAIL_LEN,
    exponent: COMET_EXPONENT, hue: COMET_HUE,
  });
}
...
// Flash de confirmación al terminar de medir — verde si salió bien, rojo si
// falló — y después vuelve solo al idle normal (breathe/wifiWeak/error según
// corresponda en ese momento).
function calibrationDone(ok = true) {
  if (!ws281x) return;
  const from = _lastRGB;
  const to   = ok ? { r: 0, g: 255, b: 0 } : { r: 255, g: 0, b: 0 };
  _fadeTo(from.r, from.g, from.b, to.r, to.g, to.b, 250, () => {
    setTimeout(() => _fadeTo(to.r, to.g, to.b, 0, 0, 0, 400, _renderIdle), 500);
  });
}
