...
// El cometa de "arrancando" (booting) usa el MISMO color que la respiración
// (IDLE_HUE/IDLE_SAT — tu "Apariencia" elegida), no un tono fijo aparte —
// así se siente como Brumexa despertando, no como un estado ajeno a la
// paleta del dispositivo. Lo que lo distingue de la respiración normal es
// la FORMA (cometa en movimiento, no un pulso uniforme quieto en el lugar),
// igual que connecting()/calibrating() ya se distinguen entre sí por forma
// y no solo por color. Más rápido/corto que COMET_PERIOD_MS: es lo primero
// que se ve apenas init() corre, antes de que WiFi/audio/calibración
// terminen de armarse, así que tiene que leerse como "algo ya está
// pasando" de un vistazo.
...
// Cometa de "midiendo" — más calmo que el de booting() (BOOT_PERIOD_MS):
// este corre varios segundos seguidos seguros (dura lo que tarda
...
// ─── Arrancando: cometa en tu color de respiración, más rápido y corto que
// connecting() ────────────────────────────────────────────────────────────
// Lo primero que se ve tras init() — todavía no hay WiFi ni calibración ni
// sesión. Se distingue del idle real por la FORMA (cometa en movimiento) y
// no por el color, así que se siente como Brumexa despertando en tu propia
// paleta, no como un estado genérico ajeno al dispositivo. Se llama desde
...
// primero que ve el usuario después de enchufar.
function booting() {
  _startComet({
