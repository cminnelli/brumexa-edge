...
function _createSpeechTracker(label, getHue, getSat, levelGain, hangoverMs) {
  ...
  //
  // Dos etapas de suavizado en cascada (en vez de una sola) — una sola
  // exponencial pega su salto más grande justo al arrancar cada objetivo
  // nuevo y después desacelera; con el nivel de audio actualizándose cada
  // ~100ms (bastante más rápido de lo que tarda en asentar), eso se veía
  // como un salto seguido de una mesetita plana, no como una curva continua.
  // Encadenar una segunda etapa sobre la primera da una curva en "S" (arranca
  // despacio, acelera, frena suave al llegar) — sube y baja paulatinamente,
  // nunca de golpe ni en escalón.
  //
  ...
  // tick de este loop pisaría el brillo real recién logrado por el fade y se
  // veía un parpadeo hacia el piso antes de volver a subir.
  function loop(startBrightness) {
    if (!ws281x) return;
    _stopBreathe();
    let smoothed1 = startBrightness;
    let smoothed2 = startBrightness;
    _breathe = setInterval(() => {
      const target = _speechBrightness(levelLive, levelGain);
      const rate   = target > smoothed1 ? SPEECH_ATTACK : SPEECH_RELEASE;
      smoothed1 += (target - smoothed1) * rate;
      smoothed2 += (smoothed1 - smoothed2) * rate;
      const { r, g, b } = hsvToRgb(getHue(), getSat(), smoothed2);
      _render(r, g, b);
    }, 16);
  }
