        ...
        if (Date.now() - lastLog > STATS_TICK_MS) {
          ...
          if (talking !== this._micTalking) {
            ...
            if (talking) {
              ...
              // Configuración → Sonido) — esto es lo único que cruza esa línea
              // y se manda de verdad a LiveKit; por debajo, se sigue mandando
              // audio (el stream nunca corta), pero no hay señal real, solo piso.
