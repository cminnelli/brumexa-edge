...
class LiveKitSession extends EventEmitter {
  ...
  async start({ token, url, roomName, micDevice, speakerDevice }) {
    ...
    try {
      ...
      console.log(`[lk-session-debug] connectionState inicial → ${this.room.connectionState}`); // DEBUG TEMPORAL
        ...
        if (Date.now() - lastLog > STATS_TICK_MS) {
          ...
          if (talking !== this._micTalking) {
            ...
            if (talking) {
              ...
              console.log(`🎤 Vos → empezaste a hablar  (raw=${rawDbfs.toFixed(1)}dBFS  muteLevantadoHace=${msSinceMuteLifted === null ? 'nunca' : msSinceMuteLifted + 'ms'})`);
            } else {
              console.log('🎤 Vos → dejaste de hablar');
            }
          }
          ...
          if (Date.now() - lastAmbientLog > 1000) {
            console.log(`[lk-session-debug] mic ambient: ${dbfs.toFixed(1)}dBFS  muted=${speakerActive}`);
  ...
  _wireRoomEvents() {
    ...
    // DEBUG TEMPORAL — el "conectado" de room.connect() es solo señalización;
    // esto muestra el estado real del transporte de medios (ICE/PeerConnection).
    // Si el audio del agente nunca llega (0 frames) pero el navegador sí lo
    // escucha en la misma sala, acá deberíamos ver si el cliente nativo se
    // queda en "reconnecting"/"disconnected" en vez de "connected".
