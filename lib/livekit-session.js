...
let SILENCE_DISCONNECT_MS = parseInt(process.env.SILENCE_DISCONNECT_MS, 10) || 10000;
  ...
  async _publishMic(device) {
    ...
    proc.stdout.on('data', async (chunk) => {
      ...
      while (buffer.length >= MIC_FRAME_BYTES) {
        ...
        if (this._micGateEnabled && !gateOpen) {
        ...
        if (Date.now() - lastLog > STATS_TICK_MS) {
          ...
          micGate.feed(level);
          const nowGateOpen = micGate.isOpen();
          if (nowGateOpen && !gateOpen) pendingPrerollFlush = true;
          if (!nowGateOpen && gateOpen) console.log('[lk-session] 🔒 Gate CERRADO — atenuando salida');
          gateOpen = nowGateOpen;
          this._micGateOpen = gateOpen;
          ...
          if (Date.now() - lastAmbientLog > 1000) {
            ...
            console.log(`[lk-session-debug] mic ambient: ${dbfs.toFixed(1)}dBFS  muted=${speakerActive}  gate=${gateOpen ? 'OPEN' : 'CLOSED'}  ambientFloor=${ambientFloor === null ? 'n/a' : ambientFloor.toFixed(1) + 'dBFS'}  effectiveThreshold=${micGate.getEffectiveThresholdDbfs().toFixed(1)}dBFS`);
