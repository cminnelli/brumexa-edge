...
class LiveKitSession extends EventEmitter {
  constructor() {
    ...
    this._speakerActiveUntil = 0;     // timestamp hasta el cual el mic está muteado
    this._micMutedLogged     = false; // para loguear transiciones de mute
    ...
    try {
      for await (const frame of stream) {
        ...
        if (this.aplayProc && this.aplayProc.stdin.writable) {
          ...
          // Calcular peak del frame para saber si tiene audio real o es silencio
          let framePeak = 0;
  ...
  async _triggerReconnect(reason) {
    ...
    console.log(`[lk-session] ↻ Reconnect ${attempt}/${AGENT_MAX_RETRIES}  razón=${reason}`);
    this.emit('reconnecting', { attempt, reason });

    // Guardar args antes de que stop() los limpie
    const args = this._startArgs;
    await this.stop().catch(() => {});
