...
class LiveKitSession extends EventEmitter {
  ...
  async _publishMic(device) {
    ...
    // Si hablás justo al límite del umbral, el gate puede abrirse y cerrarse
    // varias veces seguidas en poco tiempo — cada apertura pedía un flush
    // nuevo, y como el flush ahora tarda hasta ~500ms en vaciarse (ver el
    // espaciado agregado más abajo), una segunda apertura ANTES de que el
    // primero terminara arrancaba un SEGUNDO flush en paralelo — dos loops
    // mandando captureFrame() al mismo audioSource a la vez, pisándose entre
    // sí. El espaciado por sí solo no alcanza para evitar esto (solo ordena
    // los frames DENTRO de un mismo flush) — este flag bloquea que arranque
    // uno nuevo mientras el anterior sigue en curso.
    let prerollFlushInFlight = false;
    const gateScratch = new Int16Array(MIC_FRAME_SAMPLES);

    // Serializa el procesamiento de cada chunk — sin esto, Node dispara este
    // listener de nuevo por cada 'data' que llega SIN esperar a que el
    // anterior termine. Antes (cuando cada captureFrame() resolvía casi al
    // instante) nunca se notaba, pero ahora que un flush del colchón puede
    // tardar hasta ~500ms de verdad (ver el espaciado más abajo), llegaban
    // chunks nuevos DURANTE ese medio segundo y su propio procesamiento
    // (captureFrame de audio en vivo) corría en paralelo con el flush,
    // pisándose los dos contra el mismo audioSource — la causa real de que
    // el error siguiera apareciendo incluso con el espaciado y el guard de
    // flushes superpuestos ya puestos (los dos evitaban que EL FLUSH se
    // pisara consigo mismo, no que un chunk normal se cruzara por encima).
    // Con esto, cada chunk espera su turno: el siguiente no arranca hasta
    // que el anterior (flush incluido) terminó del todo.
    ...
    const processChunk = async (chunk) => {
      ...
      while (buffer.length >= MIC_FRAME_BYTES) {
        ...
        if (pendingPrerollFlush) {
          ...
          prerollFlushInFlight = true;
          const buffered = prerollFrames.splice(0);
          console.log(`[lk-session] 🔓 Gate ABIERTO — preroll flush (${buffered.length}×${MIC_FRAME_MS}ms)`);
          // Espaciados a MIC_FRAME_MS entre uno y otro — bug real encontrado
          // acá: mandarlos todos de un saque (un await detrás de otro, sin
          // pausa real entre ellos — la promesa de captureFrame resuelve casi
          // al instante) le llega al AudioSource nativo mucho más rápido que
          // el ritmo real de audio que espera (uno cada 20ms, como si viniera
          // en vivo del mic) — lo rechaza con "RtcError InvalidState", y ESE
          // frame en particular no se manda. No era solo ruido en el log: el
          // colchón existe justo para no perder el arranque de la palabra, y
          // la mayoría de esos frames se estaban perdiendo igual por venir
          // demasiado rápido. Con la pausa, tarda hasta ~500ms en vaciarse
          // (25 × 20ms) pero SÍ llega completo.
          for (let i = 0; i < buffered.length; i++) {
            // Mismo guard que ya usa el resto de este handler (ver el
            // comentario grande más arriba, "Guard contra la carrera de
            // cierre") — acá hace falta explícito porque el flush ahora
            // tarda hasta 500ms de verdad (antes terminaba casi al
            // instante), así que un stop() a mitad de camino es mucho más
            // fácil que caiga justo en este loop.
            if (!this._micPublishing) break;
            try {
              const pf = new AudioFrame(buffered[i], MIC_SAMPLE_RATE, MIC_CHANNELS, MIC_FRAME_SAMPLES);
              await this.audioSource.captureFrame(pf);
            } catch (e) {
              console.error('[lk-session] captureFrame (preroll):', e.message);
            }
            if (i < buffered.length - 1) await new Promise(r => setTimeout(r, MIC_FRAME_MS));
          }
          prerollFlushInFlight = false;
        }
        ...
        if (Date.now() - lastLog > STATS_TICK_MS) {
          ...
          if (nowVoiceActive && !voiceActive && !prerollFlushInFlight) pendingPrerollFlush = true;
    ...
    proc.stdout.on('data', (chunk) => {
      _chunkQueue = _chunkQueue.then(() => processChunk(chunk)).catch(e => console.error('[lk-session] processChunk error:', e.message));
    });
  }
  ...
  playChime(pcmBuffer) {
    ...
    if (this.aplayProc && this.aplayProc.stdin.writable) {
      // Mismo aviso de half-duplex que ya usa la voz REAL del agente
      // (_consumeRemoteAudio, _speakerActiveUntil) — sin esto, el mic sigue
      // escuchando de lleno mientras el chime suena por el parlante, lo
      // capta acústicamente (dispositivo con mic y parlante cerca) y, al
      // ser un sonido fuerte, puede cruzar el umbral de voz y mandarse como
      // si fueras vos hablando. Duración real del PCM (bytes / 2 por
      // sample / sampleRate) + el mismo margen de cola que ya usa la voz
      // del agente, no un número fijo a ojo.
      const chimeMs = (pcmBuffer.length / 2 / SPEAKER_SAMPLE_RATE) * 1000;
      this._speakerActiveUntil = Date.now() + chimeMs + MUTE_TAIL_MS;
