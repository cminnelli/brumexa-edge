...
const SPEAKER_GAIN = parseFloat(process.env.SPEAKER_GAIN) || 1.0;
...
// Umbral simple para decidir "está hablando" vs "silencio" en los logs
// legibles de mic/parlante (no es un VAD real, solo para no imprimir en
// cada tick — logueamos únicamente cuando cambia el estado).
// Compartido entre mic y parlante. Ajustable en vivo (setTalkThreshold,
// sin reiniciar) y persistible en .env vía MIC_TALK_THRESHOLD_DBFS
// (más alto/menos negativo = necesita hablar más fuerte para contar como "hablando").
let TALKING_THRESHOLD_DBFS = parseFloat(process.env.MIC_TALK_THRESHOLD_DBFS) || -50;
...
// menos cortes audibles. Ajustable por env si hace falta afinar.
const APLAY_BUFFER_SIZE = parseInt(process.env.APLAY_BUFFER_SIZE || '48000', 10);
...
// Cubre el buffer de aplay (~500ms) + tail acústico. Ajustable con env.
const MUTE_TAIL_MS = parseInt(process.env.MIC_MUTE_TAIL_MS || '800', 10);
...
const AGENT_DETECT_TIMEOUT_MS = parseInt(process.env.AGENT_DETECT_TIMEOUT_MS || '8000', 10);
const AGENT_MAX_RETRIES       = parseInt(process.env.AGENT_MAX_RETRIES       || '3',    10);
const AGENT_RETRY_DELAY_MS    = parseInt(process.env.AGENT_RETRY_DELAY_MS    || '6000', 10);
  ...
  async _publishMic(device) {
    ...
    proc.stdout.on('data', async (chunk) => {
      ...
      while (buffer.length >= MIC_FRAME_BYTES) {
        ...
        // Stats cada 2 s — solo logueamos cuando CAMBIA el estado (empezó/dejó
        // de hablar), no cada tick, para no inundar la consola.
        if (Date.now() - lastLog > STATS_TICK_MS) {
          ...
          if (talking !== this._micTalking) {
            ...
            console.log(talking ? '🎤 Vos → empezaste a hablar' : '🎤 Vos → dejaste de hablar');
          }
    ...
    try {
      for await (const frame of stream) {
        ...
        if (this.aplayProc && this.aplayProc.stdin.writable) {
          // Aplicar SPEAKER_GAIN con clipping (si es 1.0, buf queda igual al
          ...
          if (SPEAKER_GAIN !== 1.0) {
            ...
            for (let i = 0; i < frame.data.length; i++) {
              let s = frame.data[i] * SPEAKER_GAIN;
              if (s > 32767)  s = 32767;
              if (s < -32768) s = -32768;
