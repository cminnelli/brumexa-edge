...
const { WARMUP_MS: MIC_OPEN_WARMUP_MS } = require('./mic-calibration');
...
// Cubre el buffer de aplay (~500ms) + tail acústico.
const MUTE_TAIL_MS = 800;
  ...
  async _publishMic(device) {
    ...
    proc.stdout.on('data', async (chunk) => {
      ...
      while (buffer.length >= MIC_FRAME_BYTES) {
        ...
        if (Date.now() - arecordStartedAt >= MIC_OPEN_WARMUP_MS && peak > peakAccum) peakAccum = peak;
        framesAccum++;
