    ...
    try {
      ...
      try {
        ...
        if (fullRemoteHash) {
          ...
          // mensaje del commit, así que esto es solo el hash, no el asunto).
          if (behind === 1) remoteHash = fullRemoteHash.slice(0, 7);
        }
      } catch (e) {
        // sin red / sin upstream configurado — no rompe la respuesta principal
      }

      res.json({ ok: true, hash, date, subject, behind, remoteHash });
    } catch (e) {
