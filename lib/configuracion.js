...
function setupConfiguracion(app, { lkSession, ragAuth, requestRoomToken, runCalibration, getLastCalibration, getEnvVal } = {}) {
  ...
  function runGitUpdate(res, onDone) {
    ...
    try {
      // "nice -n 19" — clonar/instalar son ráfagas cortas de CPU/disco a
      // fondo; en la Pi eso se sintió como estática en el parlante durante
      // el pico (probablemente ruido eléctrico por el consumo de golpe,
      // más que un tema de audio en sí). Bajarle la prioridad no achica el
      // trabajo en sí, pero evita que compita al máximo con todo lo demás
      // que esté corriendo en simultáneo (LEDs, audio, LiveKit) — mitiga el
      // pico, no promete eliminarlo del todo (eso sería más un tema de la
      // fuente de alimentación).
      _sh(`nice -n 19 git clone --quiet "${remoteUrl}" "${tmpDir}"`, { timeout: 120000 });
    } catch (e) {
      _rmDir(tmpDir);
    ...
    try {
      _sh('nice -n 19 npm install', { cwd: tmpDir, timeout: 240000 }); // ver comentario de "nice" en el git clone de más arriba
