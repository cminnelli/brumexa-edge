...
function setupConfiguracion(app, { lkSession, ragAuth, requestRoomToken, runCalibration, getLastCalibration, getEnvVal, leds, startMicMonitor, stopMicMonitor } = {}) {
  ...
  app.get('/configuracion/version', async (_req, res) => {
    ...
    try {
      // Async (_shAsync, no execSync) — "git ls-remote" de más abajo pega
      // contra la red de verdad (GitHub), y esto se dispara CADA VEZ que se
      // abre Configuración. Con execSync, una red lenta bloqueaba el hilo
      // principal entero (audio, LEDs) por el tiempo que tardara esa
      // consulta — visto en logs reales: 1.1s de atraso justo en este
      // momento. Los otros comandos (git log/rev-parse/config, todos
      // locales) también pasan a async por consistencia, aunque su aporte
      // al bloqueo sea mínimo comparado con el de red.
      //
      ...
      const local = await _shAsync('git log -1 --format="%h|%cI|%s"', opts);
      const [hash, date, ...rest] = local.split('|');
      const subject = rest.join('|');

      // Branch actual — se muestra en el panel ("Branch corriendo: dev/main")
      // y sirve de default para el selector de "Buscar actualizaciones" la
      // primera vez que se abre la página en un dispositivo.
      let branch = null;
      try {
        branch = await _shAsync('git rev-parse --abbrev-ref HEAD', opts);
      } catch (e) { /* sin red no debería fallar (es local), pero por las dudas no rompe la respuesta */ }

      let behind = null;
      let remoteHash = null;
      let remoteSubject = null;
      try {
        const line = await _shAsync(`git ls-remote --quiet origin refs/heads/${branch}`, { ...opts, timeout: 15000 });
        ...
        if (fullRemoteHash) {
          ...
          if (behind === 1) {
            ...
            try {
              const remoteUrl = await _shAsync('git config --get remote.origin.url', opts);
      ...
      res.json({ ok: true, hash, date, subject, branch, behind, remoteHash, remoteSubject });
    } catch (e) {
  ...
  // Pedido explícito: poder elegir "dev" (para probar cosas) o "main"
  // (estable) al actualizar, en vez de quedar pegado al branch por defecto
  // del remoto (main en GitHub). Whitelist chica en vez de aceptar
  // cualquier string — esto se interpola en un comando de shell más abajo
  // (git clone --branch "<branch>"), y aunque /terminal/run ya permite
  // correr shell arbitrario desde este mismo panel, no hay motivo para
  // aceptar acá algo que no sea un nombre de branch real.
  const ALLOWED_BRANCHES = ['dev', 'main'];
  function _sanitizeBranch(branch) {
    return ALLOWED_BRANCHES.includes(branch) ? branch : null;
  }
  ...
  async function runGitUpdate(res, onDone, branch) {
    ...
    const safeBranch  = _sanitizeBranch(branch);
    const branchFlag  = safeBranch ? `--branch "${safeBranch}"` : ''; // sin branch pedido → el default del remoto (main en GitHub), mismo comportamiento de siempre
    console.log(`[configuracion] update: clonando fresco en ${tmpDir}${safeBranch ? ` (branch "${safeBranch}")` : ''}…`);
    try {
      ...
      await _shAsync(`nice -n 19 git clone --quiet ${branchFlag} "${remoteUrl}" "${tmpDir}"`, { timeout: 120000 });
    } catch (e) {
      _rmDir(tmpDir);
    ...
    if (oldHash === newHash) {
      ...
      _rmDir(tmpDir);
      res.json({ ok: true, upToDate: true, restarting: false, branch: safeBranch });
      onDone();
      return;
    }
    ...
    res.json({ ok: true, upToDate: false, restarting: true, hash: newHash, branch: safeBranch });
    onDone();
    setTimeout(() => process.exit(0), 800);
  }

  app.post('/configuracion/update', require('express').json(), (req, res) => {
    if (process.platform !== 'linux') {
    ...
    _gitBusy = true;
    runGitUpdate(res, () => { _gitBusy = false; }, req.body?.branch);
  });
  ...
  app.post('/configuracion/update/stash-and-retry', require('express').json(), (req, res) => {
    if (process.platform !== 'linux') {
