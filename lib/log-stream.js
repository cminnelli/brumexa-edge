...
// ─── Panel "WiFi" del visor /logs ─────────────────────────────────────────────
// Mismo WebSocket (/ws/logs), mismo mecanismo de ring buffer que el panel de
// servidor de arriba, pero alimentado por lib/wifi.js (_logDebug) en vez de
// por console.log/warn/error — separado en su propio ring + campo `source`
// para que el navegador los pinte en un panel aparte sin mezclarse con el
// resto del log del servidor. wifiRing arranca sembrado con las últimas
// líneas del archivo persistente (logs/wifi-debug.log, ver seedWifiRing en
// wifi.js) para no arrancar en blanco tras un reinicio — después de eso
// crece en vivo igual que `ring`.
const WIFI_RING_SIZE = 300;
const wifiRing = [];
...
function _push(stream, stage, origin, text) {
  const entry = { ts: Date.now(), stream, text, stage, origin, source: 'server' };
  ring.push(entry);
...
// Llamado desde lib/wifi.js (_logDebug) por cada línea que escribe al archivo
// persistente — así el panel "WiFi" de /logs la recibe en vivo, sin exponer
// el archivo por ninguna ruta HTTP navegable.
function pushWifiEntry(text, ts) {
  const entry = { ts: ts || Date.now(), stream: 'stdout', text, stage: null, origin: 'PI', source: 'wifi' };
  wifiRing.push(entry);
  if (wifiRing.length > WIFI_RING_SIZE) wifiRing.shift();
  _broadcast(entry);
}

// Llamado una sola vez al boot (lib/wifi.js) con las últimas líneas ya
// escritas en logs/wifi-debug.log, para que el panel "WiFi" no arranque
// vacío después de un reinicio — no vuelve a leer el archivo después de esto,
// solo lo siembra; de ahí en más el ring crece con pushWifiEntry() en vivo.
function seedWifiRing(entries) {
...
module.exports = { setupLogStream, handleLogsWsConnection, pushWifiEntry, seedWifiRing };
